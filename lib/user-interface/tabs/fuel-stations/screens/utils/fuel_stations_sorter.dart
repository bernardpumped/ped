...
class FuelStationsSorter {
  ...
  int _comparisonResultForPriceSortOrder(final FuelStation fs1, final FuelStation fs2, final String? fuelType) {
    ...
    if (comparisonResult == 0) {
      comparisonResult = _comparePromos(fs1, fs2);
    }
    return comparisonResult;
  }
  ...
  int _comparisonResultForDistanceSortOrder(final FuelStation fs1, final FuelStation fs2, final String? fuelType) {
    ...
    if (comparisonResult == 0) {
      comparisonResult = _comparePromos(fs2, fs1);
    }
    return comparisonResult;
  }
  ...
  int _comparePromos(fs1, fs2) => fs2.hasPromos ? 1 : (fs1.hasPromos ? -1 : 0);

  int _compareDistance(fs1, fs2) => fs1.distance.compareTo(fs2.distance);

  int _compareFuelQuotes(fs1, fs2, fuelType) {
    if (fuelType == null) {
      return 0;
    }
